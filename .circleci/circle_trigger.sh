...
# Script from: https://github.com/labs42io/circleci-monorepo/blob/master/.circleci/circle_trigger.sh
# To be able to trigger workflows via API, you need a CircleCI personal API token.
# The circle_trigger.sh script expects to find the token in CIRCLE_TOKEN environment variable.
# To prevent having the tokens published to git, you can use project environment variables.
